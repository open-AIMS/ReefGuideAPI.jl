...
function setup_tile_routes()
    @get "/to-tile/{zoom}/{lon}/{lat}" function (req::Request, zoom::Int64, lon::Float64, lat::Float64)
        x, y = _lon_lat_to_tile(zoom, lon, lat)
        return json(Dict(:x=>x, :y=>y))
    end

    @get "/to-lonlat/{zoom}/{x}/{y}" function (req::Request, zoom::Int64, x::Int64, y::Int64)
